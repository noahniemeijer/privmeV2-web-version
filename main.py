...
def run_client(stdscr, privateKey, publicKey):
    ...
    nc.draw_text(stdscr, maxY // 2, maxX // 2 - (len("host:") // 2), "host:")
    host = nc.get_input(stdscr, maxY // 2 + 1, maxX // 2 - (len("host:") // 2) - 1, "")

    nc.clear_block(stdscr, maxY // 2, maxY // 2 + 1, 0, 0)
    nc.draw_text(stdscr, maxY // 2, maxX // 2 - (len("port:") // 2), "port:")
    port = int(
        nc.get_input(stdscr, maxY // 2 + 1, maxX // 2 - (len("port:") // 2) - 1, "")
    )
    ...
    _, info = recv_server(clientSocket, privateKey)

    if info == "BAD":
        exit(1)

    nc.clear_block(stdscr, 4, maxY - 4, 0, 0)
...
def username_transfer(stdscr, clientSocket, privateKey, serverKey):
    ...
    while usernameStatus != "VALID":
        ...
        nc.draw_text(
            stdscr, maxY // 2, maxX // 2 - (len("username:") // 2), "username:"
        )
        username = nc.get_input(
            stdscr, maxY // 2 + 1, maxX // 2 - (len("username:") // 2) - 1, ""
        )
...
def group_transfer(stdscr, clientSocket, privateKey, serverKey):
    ...
    while 1:
        ...
        action = nc.get_input(stdscr, maxY // 2 + row + 1, maxX // 2 - 4, "")
        ...
        if action != "create":
            ...
            nc.clear_block(stdscr, maxY // 2 - 2, maxY // 2 + 2, 0, 0)
            ...
            if status == "BAD":
                ...
                password = nc.get_input(
                    stdscr, maxY // 2 + 1, maxX // 2 - (len("password: ") // 2), ""
                )
            ...
            password = nc.get_input(
                stdscr, maxY // 2, maxX // 2 - (len("password:") // 2), "password:"
            )
            if password != "":
                password = ae.generate_hash(password)
            send_server(clientSocket, password, serverKey)  # 7
